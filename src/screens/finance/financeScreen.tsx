...
import {
  getPipeCustomDateString,
  getPipeTransformDateStringNumber,
  getPipeTransformDateStringPT,
  validateDateString
...
import ActionModalCalculator from '../../components/actionModalCalculadora';
...
const FinanceScreen: React.FC = () => {
  ...
  const [isTextInputEditable, setIsTextInputEditable] = React.useState<boolean>(true);
  ...
  const saveFinance = async () => {
    try {
      ...
      if (id) {
        ...
        console.log(' update (FinanceScreen): Body', body);

      } else {
      try{
        await api.post('/receive', {
          description: body.description,
          value: Number(body.money.replace(',', '.')),
          type:  body.type === 'INCOME' ? 'receita' : 'despesa',
          date: body.paid
        })
        
      }
      catch(err: any){
        console.error('Erro ao salvar dados:', err.response);
      }
        await AppFinanceService.create(body);
        console.log(' create (FinanceScreen): Body', body);
      ...
      console.error('Error saveFinance (FinanceScreen):', error);
  ...
  const deleted = () => {
    try {
      ...
      console.error('Error deleted (FinanceScreen):', error);
    }
  }
  ...
  const handleConfirmExpression = (value: string) => {
    
   
    console.log('Valor da calculadora:', value);
    

    // Atualiza o valor do campo quando a calculadora confirma
    setFinanceForm((prev) => ({ ...prev, money: value }));
    console.log('Novo Valor da calculadora:', value);
   
    console.log('Formulário atualizado:', financeForm);
  ...
  return (
    <SafeAreaView style={style.container}>
      ...
      <View style={style.containerValue}>
        ...
        <TextInputMask
          ...
          onChangeText={(money: string) =>
            isTextInputEditable ? setFinanceForm((prev) => ({ ...prev, money })) : null
          }
      ...
      <Modal
        ...
        onRequestClose={() => setIsCalculatorModalVisible(false)}
      >
      <ActionModalCalculator handleClose={handleConfirmExpression} />
      </Modal>
